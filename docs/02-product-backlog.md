...
- `[NOTA]` US-INV-07: **equipos usados** implementados — tabla `equipos_usados` (origen, cliente origen, valor de parte de pago), `POST/GET/PUT /usados` (admin), estado derivado del stock (disponible/vendido) y página `/usados`. Diferido: garantía automática al vender usados, parte de pago en POS, alta desde órdenes, usados en el historial del cliente.
- `[NOTA]` BR-GAR-06: **garantía automática por venta de producto** implementada — al vender con cliente se genera una garantía por producto distinto (`producto_nuevo` 30d / `usado` 15d); las ventas a mostrador no generan garantía.
...
- `[NOTA]` US-VEN-11/12: **cancelación y devolución de ventas con UI** implementadas — botones "Cancelar venta" (solo admin, motivo obligatorio, oculto en canceladas) y "Devolver" (vendedor/admin, cantidades parciales por línea, motivo opcional) en el ticket de Ventas. Reglas: se bloquea cancelar/devolver una **venta a crédito con abonos cobrados** (`SALE_WITH_PAYMENTS`); la devolución restituye stock y marca la venta `devuelta` completa (sin acumulado parcial posterior); el **reembolso/nota de crédito queda diferido** (se gestiona fuera del sistema). Al cancelar/devolver una venta con **parte de pago (trade-in)**, el usado se **reintegra al inventario** (queda disponible y se desvincula `venta_id=NULL` si no fue vendido). Además se corrigió el mapeo de `lineas` en el historial (`productoId`/`descripcion`/`precio`).
- `[NOTA]` US-REP-07: **export a PDF** implementado — botón "PDF" en Reportes que imprime el reporte activo (con filtros) vía `window.print()`/`@media print` (área A4), sin dependencias nuevas.
- `[NOTA]` US-INV-05: **ajustes de inventario por tipo** — `POST /productos/:id/ajustar` acepta `tipo: ajuste|merma|dano` (migración 0016: `movimiento_tipo` + `MERMA`/`DANO`); merma y daño solo con cantidad negativa.
- `[NOTA]` US-VEN-08: **notas de crédito por devoluciones** — al devolver una venta de **contado con cliente** se genera una nota (`NC-####`) por el total de líneas devueltas (precio unitario × cantidad), sin vigencia, aplicable en el POS a ventas de contado del mismo cliente (`notaCreditoId` reduce `totalAPagar` y consume saldo; si cubre todo, la venta queda sin pago en efectivo). Migración `0017_notas_credito`. Las **ventas a crédito devueltas salen de la CxC** (estado `devuelta` excluido en cartera, cliente, dashboard y reports). El corte desglosa `notasCredito` como ingreso no monetario.
- `[NOTA]` US-VEN-08: **reembolso en efectivo en devoluciones** — el vendedor elige "Reembolso en efectivo" o "Nota de crédito" (nota solo con cliente; mostrador siempre reembolsa en efectivo); el reembolso se registra como **egreso** (`categoria='Reembolso'`, efectivo) en la **caja abierta del día** (reduce el `esperadoEfectivo` del corte; se permite en negativo). Solo contado; `REEMBOLSO_INVALIDO`/`CAJA_NOT_OPEN`.
- `[NOTA]` US-VEN-14: **abonos mixtos en crédito** — `POST /ventas/:id/pagos` acepta `{ pagos: [{ metodo, monto }] }` (máx 5, Σ ≤ pendiente) además del formato simple; UI con desglose en el diálogo de abono (Cliente y Finanzas).
- `[NOTA]` US-COM-05: **proveedor más barato en reabastecimiento** — sin favorito se elige el proveedor de menor último precio **activo** (se saltan inactivos; favorito inactivo cae al más barato activo); flag `esMasBarato` por línea y badge en Reabastecimiento. Aplica también a la aprobación de solicitudes.
- `[NOTA]` NOT-01..04: **WhatsApp real (Twilio)** implementado — canal automático por `preferencia_contacto` (whatsapp → WhatsApp con fallback a correo; correo → correo; llamada → tarea manual), teléfonos normalizados a E.164 (`TWILIO_DEFAULT_COUNTRY_CODE`, default 52) y envío simulado sin credenciales `TWILIO_*`. Los workers de retraso (NOT-01) y garantía (NOT-04) usan la misma regla.
- `[NOTA]` **YunoHost empaquetado** — paquete `Magh97/techstore_ynh` (instalación nativa Node 22 + systemd + PostgreSQL + nginx; SPA con proxy /api; `SEED_DEMO=false`; SMTP relay local; Twilio opcional). El repo `techfix-product` pasó a **público** para la descarga del source. Sin SSO (login propio).
- `[NOTA]` **Primer admin en producción** — con `SEED_DEMO=false` el seed crea el admin inicial (`admin`) con contraseña generada en los logs del primer arranque (idempotente); el operador la cambia al entrar. Elimina el bloqueo de primer acceso sin usuarios demo.
